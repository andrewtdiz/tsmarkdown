...
export function activate(context: vscode.ExtensionContext) {
  ...
  const compileCommand = vscode.commands.registerCommand('better-mdx.compile', async (uri?: vscode.Uri) => {
  ...
  const executeCommand = vscode.commands.registerCommand('better-mdx.execute', async (uri?: vscode.Uri) => {
    const activeEditor = vscode.window.activeTextEditor;
    const fileUri = uri || (activeEditor?.document.uri);
  ...
  const previewCommand = vscode.commands.registerCommand('better-mdx.preview', async (uri?: vscode.Uri) => {
    const activeEditor = vscode.window.activeTextEditor;
    const fileUri = uri || (activeEditor?.document.uri);
  ...
  const completionProvider = vscode.languages.registerCompletionItemProvider(
    { scheme: 'file', language: 'better-mdx' },
