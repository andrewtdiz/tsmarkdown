...
export async function executeMDXTemplate(compiled: CompiledMDX, context: TemplateContext = {}, props: any = {}, basePath?: string): Promise<TemplateExecutionResult> {
  const errors: string[] = [];
...
async function processMultipleReturnStatements(
...
function processConditionalBlocksForParsing(
  content: string,
  conditionalBlocks: Array<{ condition: string; content: string }>,
): string {
...
function processTernaryExpressionsForParsing(
  content: string,
  ternaryExpressions: Array<{ condition: string; trueValue: string; falseValue: string }>,
): string {
...
function processJSXElementsForParsing(
  content: string,
  jsxExpressions: Array<{ placeholder: string; expression: string }>,
): string {
...
function processJSXExpressionsForParsing(
  content: string,
  jsxExpressions: Array<{ placeholder: string; expression: string }>,
): string {
...
function loadDependencies(dependencies: string[], basePath: string, errors: string[]): void {
...
function resolveComponentPath(componentName: string, basePath: string): string | null {
...
function createPropsContext(functionParams: string[], props: any, parameterTypes?: Array<{ name: string; type: string; required: boolean }>): TemplateContext {
...
function mergePropsWithDefaults(jsxProps: any, compiledComponent: CompiledMDX): any {
...
async function executeTypeScript(typescript: string, context: TemplateContext): Promise<any> {
  if (!typescript.trim()) {
    return {};
  }
...
function createSafeContext(context: TemplateContext, typescript?: string, resolvedImports?: any): any {
...
async function resolveImports(imports: string[]): Promise<any> {
...
function extractVariableNames(typescript: string): string[] {
...
function processInterpolations(
  content: string,
  interpolations: Array<{ placeholder: string; expression: string }>,
  context: any,
  errors: string[]
  ...
  for (let i = 0; i < ternaryExpressions.length; i++) {
    ...
    try {
      ...
      if (processedValue && typeof processedValue === 'string') {
        ...
        processedValue = processInterpolations(processedValue, [], context, errors);
      }
...
async function processJSXElements(
  content: string,
  jsxExpressions: Array<{ placeholder: string; expression: string }>,
  context: any,
  errors: string[],
...
async function processJSXExpressions(
  content: string,
  jsxExpressions: Array<{ placeholder: string; expression: string }>,
  context: any,
  errors: string[]
...
async function evaluateJSXExpression(expression: string, context: any): Promise<any> {
  try {
...
async function evaluateTernaryExpression(expression: string, context: any): Promise<any> {
...
async function evaluateMapExpression(expression: string, context: any): Promise<string> {
...
async function evaluateTernaryJSXExpression(expression: string, context: any): Promise<string> {
...
async function renderJSXElement(
  ...
  if (componentRegistry[componentName]) {
    try {
      ...
      const componentResult = await executeMDXTemplate(componentRegistry[componentName], context, finalProps);
...
async function renderJSXComponent(jsxElement: string, context: any): Promise<string> {
  ...
  if (componentRegistry[componentName]) {
    try {
      ...
      const componentResult = await executeMDXTemplate(componentRegistry[componentName], {}, mergedProps);
      return componentResult.content;
    } catch (error) {
...
function jsxResultToString(result: any): string {
