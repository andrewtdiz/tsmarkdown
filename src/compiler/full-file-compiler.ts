...
import { preprocessTSmdInFunctions } from './multi-function-compiler';
import { extractVariableValues, processGlobalTemplates } from './processVariables';
 ...
 */
export function compileFullFile(source: string): FullFileCompilationResult {
    const errors: string[] = [];
...
export function transpile(source: string): string {
    const { transpiledFile, errors } = compileFullFile(source);
